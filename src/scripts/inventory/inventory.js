...
(function () {
    ...
    // default options for inventory feature
    var options = {
        tryGlobal: true,
        defaultStrings: {
    ...
    // attach an event to the inventory window
    function _attachEvent(inv, loc, items, cont) { // attach special inventory change events to the document
        $(document).trigger({
    ...
    function _getUID(name, i) { // generates a random, unique-enough-for-this-purpose ID
        var key = Math.random().toString(36).substring(7);
        if (arguments.length < 2) { // we'll generate an id anyway, but it might not be as unique as i'd like.
    ...
    function Inventory(array) {
        ...
        if (array) { // some number of arguments were passed
        ...
        // if inventory is an array attach event
    ...
    Object.assign(Inventory, {
        // is inventory or not
        is: function (inv) { // test to see if passed object is an inventory instance
            return(inv instanceof Inventory);
        },
        // takes an inventory and returns a string
        log: function (inv) { // logs the inventory to the console (for debugging)
            var string = (Inventory.is(inv)) ? ('Inventory.log() -> ' + inv.toArray().join(' - ')) : ('Inventory.log() -> object is not an inventory...');
            return string;
        },
        // remove duplicated items from inventory
        removeDuplicates: function (inv) { // removes duplicate items from an inventory
            // do nothing if inventory is not inv
            if (! Inventory.is(inv)) {
                return;
            }
            // convert inventory to array
            var items = inv.toArray();
            // remove duplicates from inventory items
            var uniq = (function (i) {
                var ret = [];
    ...
    Object.assign(Inventory.prototype, {
        // transfers items to the given location
        transfer: function (loc) { // transfer items from this inventory to a specified inventory
            ...
            // array of pushed inventory items
            var pushed = [];
            // remove items that are already in inv
            for (var i = 0, l = items.length; i < l; i++) { // only transfer the item if it exists
        ...
        // returns if there are any items in inventory
        ...
        // pick up items in inventory
        pickUp: function (unique) { // add items to this inventory
            // flatten the arguments
            var items = [].slice.call(arguments).flatten(),
                inventory = this;
            // attach items to inventory
            if (items && items.length) {
        ...
        drop: function () { // remove items from this inventory
            var items = [].slice.call(arguments).flatten(),
                inventory = this,
                del; // we need to access this in the <array>.forEach()
            if (items && items.length) {
                var moved = [];
                items.forEach(function (item) {
        ...
        // sorts the inventory based on the current state
        ...
        // returns empty string if no separator is passed
        ...
        count: function (item) { // if item, count the number of duplicates
        ...
        // create a linked list for the given location
        linkedList: function (loc, action) { // construct the list elements to keep the macro call clean-ish
            ...
            list.forEach(function (item, idx, arr) {
                ...
                $link // create the drop link.wiki(drop).addClass('simple-inv drop-link');
                ...
                $listing.attr('id', UID).addClass('simple-inv link-listing').wiki(item + ' ').append($link);
        ...
        toJSON: function () {
            // the custom revive wrapper for SugarCube's state tracking
    ...
    if (options.tryGlobal) { // this allows authors to access the JS API using the global Inventory function
    ...
    Macro.add('newinventory', {
        handler: function () {
            ...
            // get the var name from the (first argument given) args[0]
    ...
    Macro.add('pickup', {
        handler: function () {
            ...
            // get the var name from the args
            var varName = this.args[0].trim();
            // check variable string
            // return error if variable is empty
            if (varName[0] !== '$' && varName[0] !== '_') {
            ...
            // get the inventory from the wikifier object
            var inv = Wikifier.getValue(varName);
            // check variable is an inventory
            if (! Inventory.is(inv)) {
    ...
    Macro.add('drop', {
        handler: function () {
            ...
            // drop the item that is = to the first argument given to macro inventory
    ...
    Macro.add('inventory', {
        handler: function () {
            ...
            $wrapper.wiki(inv.show(sep)).addClass('macro-' + this.name).appendTo(this.output);
        }
    });
    ...
    Macro.add('linkedinventory', {
        handler: function () {
            ...
            if (this.args.length > 2) { // the receiving inventory, if applicable
            ...
            $list.attr({ // add data to reconstruct the call via an event
                id: varID,
                'data-rec': recVarName,
                'data-self': varName,
                'data-action': action
